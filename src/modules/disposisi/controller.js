...
const ubahStatus = async (req, res) => {
  ...
  try {
    const disposisi = await db('disposisi')
      ...
      .join('pegawai', 'disposisi.kepada_pegawai_id', 'pegawai.id')
      .join('surat_masuk', 'disposisi.surat_masuk_id', 'surat_masuk.id')
      .select('disposisi.*', 'pegawai.user_id', 'surat_masuk.perihal', 'surat_masuk.nomor_agenda')
      .first();
    ...
    const updatedDisposisi = await db('disposisi')
      .where('disposisi.id', id)
      .join('surat_masuk', 'disposisi.surat_masuk_id', 'surat_masuk.id')
      .join('pegawai as pemberi', 'disposisi.dari_pegawai_id', 'pemberi.id')
      .join('pegawai as penerima', 'disposisi.kepada_pegawai_id', 'penerima.id')
    ...
    const terlambatBaru =
      updatedDisposisi.batas_waktu &&
      new Date(updatedDisposisi.batas_waktu) < new Date() &&
      updatedDisposisi.status !== 'selesai';

    return res.status(200).json({
      success: true,
      message: `Status disposisi berhasil diubah menjadi ${status}`,
      data: {
        id: updatedDisposisi.id,
        status: updatedDisposisi.status,
        terlambat: terlambatBaru,
        instruksi: updatedDisposisi.instruksi,
        batas_waktu: updatedDisposisi.batas_waktu,
        hasil_tindak_lanjut: updatedDisposisi.hasil_tindak_lanjut,
        pemberi: {
          id: updatedDisposisi.dari_pegawai_id,
          nama: updatedDisposisi.pemberi_nama,
          role: updatedDisposisi.pemberi_role
        },
        penerima: {
          id: updatedDisposisi.kepada_pegawai_id,
          nama: updatedDisposisi.penerima_nama
        },
        dibuat_pada: updatedDisposisi.created_at,
        dibaca_pada: updatedDisposisi.dibaca_pada,
        selesai_pada: updatedDisposisi.selesai_pada,
        surat: {
          id: updatedDisposisi.surat_id,
          nomor_agenda: updatedDisposisi.nomor_agenda,
          perihal: updatedDisposisi.perihal,
          pengirim: updatedDisposisi.pengirim,
          berkas: updatedDisposisi.file_name ? { nama: updatedDisposisi.file_name } : null
        }
      }
    });
...
const riwayat = async (req, res) => {
  ...
  try {
    ...
    const rows = await db('riwayat_disposisi')
    ...
    const data = rows.map((r) => ({
      ...
      catatan: r.catatan,
      aktor: { id: r.aktor_id, nama: r.aktor_nama },
      waktu: r.waktu
    }));
